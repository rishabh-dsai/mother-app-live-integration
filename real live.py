...
st.set_page_config(page_title='Mother App Live Integration',layout="wide",)


uploaded_file = st.file_uploader("Choose the file in requisite format")
if uploaded_file is not None:
    main=pd.read_excel(uploaded_file,sheet_name='main')
    live_birth=pd.read_excel(uploaded_file,sheet_name='Details of each Live birth')
    anc_visit=pd.read_excel(uploaded_file,sheet_name='Enter the ANC visit details')
    hbnc_visit=pd.read_excel(uploaded_file,sheet_name='HBNC Details')
    live_birth_2=pd.read_excel(uploaded_file,sheet_name='Details of Live births')
    last_2_preg_records=pd.read_excel(uploaded_file,sheet_name='Last two Pregnancy Record Grou')
